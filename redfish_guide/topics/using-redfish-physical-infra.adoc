...
that, all together, enable computers to boot into Operating Systems from the
network:
...
* The physical servers' NICs that are connected to the network boot network
  require a TFTP access for reading the initial executable file containing the
  PXE itself, and the HTTP access for downloading the Operating Systems' files.
