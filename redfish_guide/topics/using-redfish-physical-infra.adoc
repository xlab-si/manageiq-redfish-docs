...
NOTE: This guide assumes we have firmware registry already set up and registered with
{product-title_short}. Moreover, this guide assumes registry contains at least one
binary compatible with the physical server we are targeting for update.
